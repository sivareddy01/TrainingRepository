...
<li class="product__list--item"><ycommerce:testId
		code="test_searchPage_wholeProduct">

		<a class="product__list--thumb" href="${productUrl}"
		...
		<c:if test="${not empty product.deliveryType}">
							Delivery Type :${product.deliveryType}<br>
		</c:if>
		<c:if test="${not empty product.alias}">
							Alias Name :${product.alias}<br>
		</c:if>
